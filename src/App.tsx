...
const firebaseConfig = {
  apiKey: "AIzaSyAdGdJLGNOGRpUSiJlaF144PbQQP751rrk",
  authDomain: "second-mind-251bd.firebaseapp.com",
  projectId: "second-mind-251bd",
  storageBucket: "second-mind-251bd.firebasestorage.app",
  messagingSenderId: "984007520104",
  appId: "1:984007520104:web:36363c47ab1dd2f23e30dd",
};
...
export default function App() {
  ...
  // User Profile States
  const [userName, setUserName] = useState("Prateek Maurya");
  const [profilePic, setProfilePic] = useState(null);

  const [syllabusCategories, setSyllabusCategories] = useState(["Raw Backlog"]);
  const [stagingTopics, setStagingTopics] = useState([]); // Liquid Queue
  const [studyTopics, setStudyTopics] = useState([]); // Active Revisions
  const [masteredTopics, setMasteredTopics] = useState([]); // Hall of Fame

  const [wisdomCategories, setWisdomCategories] = useState(["Quick Thoughts"]);
  const [wisdomNotes, setWisdomNotes] = useState([]);
  const [expandedWisdomCategory, setExpandedWisdomCategory] = useState(null);

  const [vaultNotes, setVaultNotes] = useState([]);
  const [vaultCategories, setVaultCategories] = useState(["Others"]);
  ...
  const [globalDeadlineDays, setGlobalDeadlineDays] = useState(30);
    ...
    const initFirebase = async () => {
      try {
        ...
        if (docSnap.exists()) {
          ...
          if (d.syllabusCategories) setSyllabusCategories(d.syllabusCategories);
          if (d.stagingTopics) setStagingTopics(d.stagingTopics);
          if (d.studyTopics) setStudyTopics(d.studyTopics);
          if (d.masteredTopics) setMasteredTopics(d.masteredTopics);
          if (d.wisdomCategories) setWisdomCategories(d.wisdomCategories);
          if (d.wisdomNotes) setWisdomNotes(d.wisdomNotes);
          if (d.vaultNotes) setVaultNotes(d.vaultNotes);
          if (d.vaultCategories) setVaultCategories(d.vaultCategories);
          if (d.globalDeadlineDays) setGlobalDeadlineDays(d.globalDeadlineDays);
  ...
  useEffect(() => {
    ...
    // 1.5 second delay before saving to prevent spamming the database
              ...
              messages: [
                {
                  role: "system",
                  content:
                    'Generate exactly 10 short, brutal, hard-hitting motivational sentences (max 15 words each) to stop someone from breaking a discipline streak. Output ONLY as a JSON array of strings. Like: ["sentence 1", "sentence 2"]',
                },
              ],
  ...
  const handleAskOracle = async () => {
    ...
    try {
      ...
      setOracleResponse(
        "The Oracle's connection was disrupted. Please check your API key in Settings."
      );
    }
  ...
  const handleAddSyllabusCategory = () => {
    if (
      newSyllabusCat.trim() &&
      !syllabusCategories.includes(newSyllabusCat.trim())
    ) {
  ...
  const handleDeleteSyllabusCategory = (cat) => {
    ...
    setStagingTopics((prev) =>
      prev.map((t) =>
        t.category === cat ? { ...t, category: "Raw Backlog" } : t
      )
    );
  ...
  const markRevisionComplete = (topicId, targetDate, dayOffset) => {
    ...
    const updatedSchedule = topic.schedule.map((rev) =>
      rev.targetDate === targetDate && rev.dayOffset === dayOffset
        ? { ...rev, completed: true }
        : rev
    );

    const day30Completed = updatedSchedule.some(
      (rev) => rev.dayOffset === 30 && rev.completed
    );
    ...
    if (day30Completed) {
      setTimeout(() => {
        ...
        setMasteredTopics((prev) => [
          ...prev,
          { ...topic, masteredDate: todayStr },
        ]);
      }, 0);
    } else {
      setStudyTopics((prev) =>
        prev.map((t) =>
          t.id === topicId ? { ...topic, schedule: updatedSchedule } : t
        )
      );
    }
  };
  ...
  const handleAddWisdomCategory = () => {
    if (
      newWisdomCat.trim() &&
      !wisdomCategories.includes(newWisdomCat.trim())
    ) {
  ...
  const handleDeleteWisdomCategory = (cat) => {
    ...
    setWisdomNotes((prev) =>
      prev.map((n) =>
        n.category === cat ? { ...n, category: "Quick Thoughts" } : n
      )
    );
  ...
  const handleMoveWisdomNote = (noteId, newCategory) => {
    setWisdomNotes((prev) =>
      prev.map((n) => (n.id === noteId ? { ...n, category: newCategory } : n))
    );
  };
  ...
  const handleAddNote = async () => {
    ...
    try {
      ...
      const prompt = `
        ...
        Other unclassified ideas: ${JSON.stringify(
          othersNotes.map((n) => ({ id: n.id, text: n.text }))
        )}
      ...
      if (aiResponse.assignedCategory) {
        ...
        setVaultNotes((prev) =>
          prev.map((n) => {
            ...
            if (
              aiResponse.extractedIdsFromOthers &&
              aiResponse.extractedIdsFromOthers.includes(n.id)
            ) {
  ...
  const renderDashboard = () => {
    ...
    const pace =
      remainingChapters > 0
        ? (globalDeadlineDays / remainingChapters).toFixed(1)
        : 0;
    ...
    const todaysCustomMissions = customMissions.filter(
      (m) => m.targetDate <= todayStr && !m.completed
    );
    const quoteOfTheDay =
      MORNING_QUOTES[baseDate.getDate() % MORNING_QUOTES.length];
        ...
        <div className="bg-[#121212] border border-zinc-800 p-6 rounded-2xl shadow-xl flex flex-col gap-5 relative overflow-hidden">
          ...
          <div className="flex justify-between items-start z-10">
            <div>
              ...
              {stagingTopics.length > 0 ? (
                <h2 className="text-white text-2xl font-black tracking-tight mt-1">
                  {stagingTopics[0].title}
                </h2>
              ) : (
                <h2 className="text-zinc-600 text-xl font-bold tracking-tight mt-1 italic">
                  No active target in Queue
                </h2>
              )}
              {stagingTopics.length > 0 && (
            ...
            <div className="text-right flex flex-col items-end">
              ...
              <div
                className={`mt-2 px-3 py-1.5 rounded-lg border flex flex-col items-end ${paceStatus.bg} ${paceStatus.border}`}
              >
                <span
                  className={`text-[10px] font-black uppercase tracking-widest ${paceStatus.color}`}
                >
                  {paceStatus.text}
                </span>
        ...
        {todaysCustomMissions.length > 0 && (
          <div className="bg-zinc-900 border border-zinc-800/80 p-5 rounded-2xl shadow-xl">
            <h3 className="text-white font-bold text-lg mb-4 flex items-center gap-2">
              <CheckCircle2 className="text-indigo-400" size={20} /> Today's
              Custom Targets
            </h3>
            <div className="space-y-3">
              {todaysCustomMissions.map((mission) => (
                <div
                  key={mission.id}
                  className="flex items-center justify-between p-4 rounded-xl border bg-indigo-950/20 border-indigo-900/50 hover:bg-indigo-950/40 transition-colors"
                >
                  <h4 className="text-indigo-100 font-bold text-sm">
                    {mission.text}
                  </h4>
                  <button
                    onClick={() =>
          ...
          {todaysRevisions.length === 0 ? (
            <div className="text-center py-10 bg-zinc-950/50 border border-dashed border-zinc-800 rounded-xl">
              ...
              <p className="text-xs text-zinc-600 mt-1">
                Focus on destroying your Current Target.
              </p>
            </div>
          ) : (
            <div className="space-y-3">
              {todaysRevisions.map((rev, idx) => (
                <div
                  key={idx}
                  className={`flex items-center justify-between p-4 rounded-xl border transition-all duration-300 hover:scale-[1.01] shadow-sm ${
                    rev.isOverdue
                ...
                >
                  <div>
                    <h4 className="text-white font-bold flex items-center gap-2">
                      {rev.title}
                      {rev.isOverdue && (
                        <span className="text-[9px] bg-red-600 px-2 py-0.5 rounded-sm text-white font-black uppercase tracking-wider animate-pulse">
                          Overdue
                        </span>
                      )}
                    </h4>
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-[10px] text-zinc-400 font-bold uppercase tracking-wider bg-zinc-900 px-2 py-0.5 rounded border border-zinc-800">
                        {rev.category}
                      </span>
                      <span className="text-[10px] font-black uppercase tracking-wider text-zinc-300 bg-white/10 px-2 py-0.5 rounded">
                        Day {rev.dayOffset} Revision
                      </span>
                    </div>
                  </div>
                  <button
        ...
        {syllabusCategories.length > 1 && (
          <div className="flex flex-wrap gap-2 mb-6">
            {syllabusCategories.map((cat) => (
              <div
                key={cat}
                className="group flex items-center gap-1 bg-zinc-950 border border-zinc-800 px-3 py-1.5 rounded-lg text-xs font-bold text-zinc-400"
              >
                {cat}
                {cat !== "Raw Backlog" && (
                  <button
                    onClick={() => handleDeleteSyllabusCategory(cat)}
                    className="text-zinc-600 hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100 ml-1"
                  >
                    <Trash2 size={12} />
                  </button>
                )}
          ...
          {stagingTopics.length === 0 && (
            <div className="text-center py-10 bg-zinc-950/50 border border-dashed border-zinc-800 rounded-xl">
              <p className="text-xs text-zinc-500 font-medium">
                Queue is empty. Add chapters below.
              </p>
            </div>
          )}
          {stagingTopics.map((topic, index) => (
            <div
              ...
              className={`bg-zinc-950 p-3 rounded-xl border flex items-center justify-between group transition-colors cursor-move 
                ${
                  index === 0
                    ? "border-zinc-500 shadow-[0_0_10px_rgba(255,255,255,0.05)]"
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="font-bold text-white text-sm flex items-center gap-2">
                    {topic.title}
                    {index === 0 && (
                      <span className="text-[8px] bg-white text-black px-1.5 py-0.5 rounded font-black uppercase tracking-widest">
                        Next Target
                      </span>
                    )}
                  </h4>
                  <span className="text-[9px] font-bold uppercase tracking-wider text-zinc-500 mt-1 block">
                    {topic.category}
                  </span>
                </div>
              </div>
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="bg-zinc-900 border border-zinc-800/80 p-5 rounded-2xl shadow-xl">
        ...
        {studyTopics.length === 0 ? (
          <p className="text-sm font-medium text-zinc-500 text-center py-6 border border-dashed border-zinc-800 rounded-xl">
            No active revisions in history. Complete a topic in Syllabus first.
          </p>
        ) : (
          <div className="space-y-4">
            {studyTopics.map((topic) => (
              <div
                key={topic.id}
                className="bg-zinc-950 p-5 rounded-xl border border-zinc-800 relative overflow-hidden transition-all hover:border-zinc-700"
              >
                <div className="flex justify-between items-start mb-4">
                  <div>
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-wider text-zinc-400 bg-zinc-900 px-2 py-1 rounded-sm mt-2 inline-block">
                      {topic.category}
                    </span>
                  </div>
                  <span className="text-[10px] font-bold text-zinc-600 bg-zinc-900 px-2 py-1 rounded">
                    Init: {topic.startDate}
                  </span>
                </div>
                <div className="flex flex-wrap gap-2">
                  {topic.schedule.map((rev, i) => {
                    const isPending =
                      !rev.completed && rev.targetDate <= todayStr;
                    return (
                      <div
                        key={i}
                        className={`flex flex-col items-center justify-center py-2 px-3 rounded-xl border transition-all duration-300 ${
                          rev.completed
                      ...
                      >
                        ...
                        {rev.completed ? (
                          <Check
                            size={14}
                            className="text-zinc-900 mt-1 stroke-[3]"
                          />
                        ) : (
                          <Circle
                            size={14}
      ...
      <div className="bg-gradient-to-br from-[#1c1917] to-zinc-900 border border-[#44403c] p-6 rounded-2xl shadow-2xl relative overflow-hidden">
        ...
        <h3 className="text-[#d6d3d1] font-black text-xl mb-6 flex items-center gap-2 tracking-tight">
          <Trophy size={24} className="text-[#a8a29e]" /> Hall of Fame
          (Mastered)
        </h3>
        {masteredTopics.length === 0 ? (
          <p className="text-sm font-medium text-[#78716c] text-center py-6 border border-dashed border-[#57534e] rounded-xl">
            Check off a Day 30 revision to lock knowledge here permanently.
          </p>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {masteredTopics.map((topic) => (
              <div
                key={topic.id}
                className="bg-[#292524] p-4 rounded-xl border border-[#57534e] flex items-center gap-4 transition-transform hover:scale-[1.02]"
              >
                ...
                <div>
                  ...
                  <p className="text-[10px] font-bold uppercase tracking-wider text-[#78716c] mt-1">
                    {topic.category} • {topic.masteredDate}
                  </p>
                </div>
              </div>
            ))}
  ...
  const renderWisdom = () => {
    if (expandedWisdomCategory) {
      const filteredNotes = wisdomNotes.filter(
        (n) => n.category === expandedWisdomCategory
      );
      return (
        <div className="space-y-6 animate-in fade-in duration-300">
          <div className="flex items-center gap-4 mb-4">
            <button
              onClick={() => setExpandedWisdomCategory(null)}
              className="p-2 bg-zinc-900 border border-zinc-800 hover:bg-zinc-800 hover:text-white rounded-lg text-zinc-400 transition-colors active:scale-95"
            >
              <ChevronLeft size={20} />
            </button>
          ...
          <div className="grid gap-4 mt-6">
            {filteredNotes.length === 0 && (
              <p className="text-zinc-500 font-medium text-center py-10 border border-dashed border-zinc-800 rounded-2xl">
                Folder is empty.
              </p>
            )}
            {filteredNotes.map((note) => (
              <div
                key={note.id}
                className="bg-zinc-900 border border-zinc-800 p-5 rounded-2xl flex flex-col gap-3 transition-colors group hover:border-zinc-600 shadow-sm"
              >
                ...
                <div className="flex justify-between items-center mt-2 pt-3 border-t border-zinc-800/50">
                  ...
                  <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <select
                      onChange={(e) =>
                        handleMoveWisdomNote(note.id, e.target.value)
                      }
                      value={note.category}
      ...
      <div className="space-y-8 animate-in fade-in duration-500">
        <div className="bg-indigo-950/20 border border-indigo-900/50 p-5 rounded-2xl shadow-xl relative overflow-hidden">
          ...
          <p className="text-xs font-medium text-indigo-200/60 mb-4 leading-relaxed">
            Chat with your Second Brain. The AI will synthesize answers using
            exclusively your saved Wisdom and Dump notes.
          </p>
          <div className="flex gap-2">
            <input
              type="text"
          ...
          {oracleResponse && (
            <div className="mt-4 p-4 bg-zinc-950/80 border border-indigo-900/50 rounded-xl animate-in fade-in duration-300">
              <p className="text-indigo-100 text-sm whitespace-pre-line leading-relaxed font-medium">
                {oracleResponse}
              </p>
            </div>
          )}
              ...
              <div key={cat} className="group relative">
                ...
                >
                  ...
                  <div>
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-wider text-zinc-500 mt-1 block">
                      {count} Notes
                    </span>
                  </div>
                </button>
                {cat !== "Quick Thoughts" && (
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      handleDeleteWisdomCategory(cat);
                    }}
  ...
  const renderVault = () => (
    <div className="space-y-6 animate-in fade-in duration-500">
      {/* Ask The Oracle (Mirrored from Wisdom) */}
      <div className="bg-indigo-950/20 border border-indigo-900/50 p-5 rounded-2xl shadow-xl relative overflow-hidden">
        <div className="absolute top-0 right-0 w-32 h-32 bg-indigo-600 opacity-10 rounded-full blur-3xl"></div>
        <h3 className="text-indigo-300 font-bold text-lg mb-3 flex items-center gap-2">
          <Sparkles size={20} className="text-indigo-400" /> Ask The Oracle
      ...
      <div className="bg-zinc-900 border border-zinc-800/80 p-5 rounded-2xl shadow-xl">
        <div className="flex justify-between items-center mb-2">
          <h3 className="text-white font-bold text-lg flex items-center gap-2">
            <BrainCircuit className="text-zinc-400" size={20} /> Brain Dump
            (Inbox)
          </h3>
          {isVaultSorting && (
        ...
        <p className="text-xs font-medium text-zinc-500 mb-5 leading-relaxed">
          Fast-capture raw ideas. Add 3 similar thoughts, and AI will
          automatically build a new folder for them.
        </p>
          ...
          <button
            onClick={() => {
              ...
              if (!SpeechRecognition) {
                alert(
                  "Voice typing is not supported in this browser. Please use Chrome/Edge."
                );
                return;
              }
          ...
          return (
            ...
            >
              ...
              {notesInCat.map((note) => (
                <div
                  key={note.id}
                  className="bg-zinc-950 border border-zinc-800 p-4 rounded-xl flex items-start gap-4 hover:border-zinc-700 transition-colors shadow-sm group"
                >
                  <BrainCircuit
                    size={16}
                    className="text-zinc-600 mt-1 flex-shrink-0 group-hover:text-zinc-400 transition-colors"
                  />
                  <div>
                    <p className="text-zinc-200 text-sm font-medium leading-relaxed">
                      {note.text}
                    </p>
                    <span className="text-[9px] font-bold uppercase tracking-wider text-zinc-600 mt-2 block">
                      {note.date}
                    </span>
                  </div>
                </div>
  ...
  const renderUrgeKiller = () => (
    <div className="space-y-6 animate-in fade-in duration-500 max-w-md mx-auto text-center pt-10">
      ...
      <p className="text-zinc-500 font-medium text-sm mb-10 leading-relaxed">
        About to break your streak? Want to scroll? Hit this button first to
        activate the friction zone.
      </p>
      ...
      {!isUrgeActive ? (
        ...
        <div className="bg-zinc-950 border border-zinc-800 p-8 rounded-3xl shadow-2xl relative overflow-hidden">
          ...
          <h3 className="text-zinc-400 font-black mb-6 uppercase tracking-[0.2em] text-xs">
            Friction Zone Active
          </h3>
          ...
          <div className="min-h-[80px] flex items-center justify-center">
            <p
              className="text-zinc-300 font-medium italic text-lg px-4 leading-relaxed animate-in fade-in zoom-in duration-500"
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="bg-zinc-900 border border-zinc-800 p-5 rounded-2xl shadow-xl">
        ...
        <p className="text-xs text-zinc-500 font-medium leading-relaxed mb-5">
          Paste your Groq API Key here. This key powers your Oracle,
          Auto-Sorter, and dynamic Urge Quotes. It is stored securely on your
          local device.
        </p>
        <input
          type="password"
  ...
  if (!isDataLoaded) {
    return (
      <div className="min-h-screen bg-black flex flex-col items-center justify-center text-zinc-500">
        <BrainCircuit className="animate-pulse mb-4 text-white" size={48} />
        ...
        <div className="flex justify-between items-center mb-10">
          <div>
            <h1 className="text-3xl font-black text-white tracking-tighter flex items-center gap-2">
              Apex Mind{" "}
              <span className="text-[10px] bg-white text-black px-2 py-1 rounded-sm font-black uppercase tracking-widest align-middle">
                V2.1
              </span>
            </h1>
        ...
        <div className="fixed bottom-24 left-1/2 -translate-x-1/2 w-[90%] max-w-sm z-40 flex flex-col items-end">
          {!isNightShiftOpen ? (
            ...
            <div className="w-full bg-[#1a1a2e] border border-indigo-900/50 p-5 rounded-2xl shadow-[0_0_40px_rgba(49,46,129,0.3)] animate-in zoom-in-95 duration-300 relative">
              ...
              <p className="text-xs text-indigo-200/60 font-medium mb-4">
                Add tasks for tomorrow, or pin a queue target.
              </p>
              ...
              {customMissions.filter(
                (m) => m.targetDate === addDays(todayStr, 1)
              ).length > 0 && (
                <div className="mb-4 space-y-1.5">
                  {customMissions
                    .filter((m) => m.targetDate === addDays(todayStr, 1))
                    .map((m) => (
                      <div
                        key={m.id}
                        className="text-xs font-medium text-indigo-200 bg-indigo-950/50 px-3 py-2 rounded-lg flex justify-between items-center border border-indigo-900/30"
                      >
                        <span>• {m.text}</span>
                        <button
                          onClick={() =>
                            setCustomMissions((prev) =>
                              prev.filter((task) => task.id !== m.id)
                            )
                          }
              ...
              {stagingTopics.length > 0 && (
                <>
                  <div className="text-[10px] font-black text-indigo-400/50 uppercase tracking-widest mb-2 border-t border-indigo-900/50 pt-3">
                    Pin Syllabus Target
                  </div>
                  <div className="space-y-2 max-h-32 overflow-y-auto hide-scrollbar">
                    {stagingTopics.slice(0, 3).map((topic, idx) => (
                      ...
                      >
                        <span className="font-bold text-indigo-100 text-xs truncate pr-2">
                          {topic.title}
                        </span>
      ...
      <div className="fixed bottom-0 left-0 w-full bg-black/90 backdrop-blur-xl border-t border-zinc-900 z-50 pb-safe overflow-x-auto hide-scrollbar">
        <div className="max-w-2xl mx-auto flex justify-between px-2 py-4 min-w-[320px]">
          <button
            ...
            className={`flex-1 flex flex-col items-center gap-1.5 p-2 transition-all duration-300 active:scale-95 ${
              activeTab === "dashboard"
                ? "text-white"
          ...
          >
            <Activity
              size={22}
              className={activeTab === "study" ? "stroke-[2.5]" : "stroke-2"}
            />
            <span className="text-[9px] font-black uppercase tracking-widest">
              Queue
            </span>
          ...
          >
            <History
              size={22}
              className={activeTab === "history" ? "stroke-[2.5]" : "stroke-2"}
            />
            <span className="text-[9px] font-black uppercase tracking-widest">
              History
            </span>
          ...
          >
            <Folder
              size={22}
              className={activeTab === "wisdom" ? "stroke-[2.5]" : "stroke-2"}
            />
            <span className="text-[9px] font-black uppercase tracking-widest">
              Wisdom
            </span>
          ...
          >
            <BrainCircuit
              size={22}
              className={activeTab === "vault" ? "stroke-[2.5]" : "stroke-2"}
            />
            <span className="text-[9px] font-black uppercase tracking-widest">
              Dump
            </span>
          ...
          >
            <ShieldAlert
              size={22}
              className={activeTab === "urge" ? "stroke-[2.5]" : "stroke-2"}
            />
            <span className="text-[9px] font-black uppercase tracking-widest">
              Urge
            </span>
